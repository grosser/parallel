...
describe Parallel do
  ...
  describe ".in_processes" do
    ...
    it 'does not leave processes behind while running' do
      ...
      `ruby spec/cases/closes_processes_at_runtime.rb`.gsub("deprecated; use BigDecimal() method instead.\n", '').should == 'OK'
    end
  ...
  describe ".each" do
    ...
    worker_types.each do |type|
      it "works with SQLite in #{type}" do
        `WORKER_TYPE=#{type} ruby spec/cases/each_with_ar_sqlite.rb 2>&1`.gsub("deprecated; use BigDecimal() method instead.\n", '').should == "Parent: X\nParallel (in_#{type}): XXX\nParent: X\n"
      end
